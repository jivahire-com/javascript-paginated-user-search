...
export class UserSearch {
  constructor(opts) {
    ...
    this._fetchId = 0; // monotonically incremented; guards against stale responses
  }
  ...
  setQuery(q) {
    ...
    // FIX Bug 1: read this.page and this.query at fire time, not at schedule time.
  ...
  getState() {
    return {
      ...
      users: this.users,
      total: this.total,
      // FIX Bug 3: ceil so a trailing partial page is always reachable.
  ...
  dispose() {
    ...
    // Invalidate any in-flight fetch so its response is silently dropped.
    this._fetchId++;
    this.listeners.clear();
  }

  async _runFetch(query, page) {
    // FIX Bug 2: stamp this fetch; only the highest stamp may commit results.
    ...
    // Discard the response if a newer fetch has already started.
