...
export class UserSearch {
  constructor(opts) {
    ...
    this._fetchGeneration = 0; // incremented on every fetch; stale responses are discarded
  }
  ...
  setQuery(q) {
    ...
    // Fix: read this.query and this.page at fire-time, not at schedule-time.
  ...
  getState() {
    return {
      ...
      users: this.users,
      total: this.total,
      // using math.ceil in place of math.floor 
  ...
  dispose() {
    ...
    // Invalidate any in-flight fetch so it cannot commit after dispose.
    this._fetchGeneration++;
    this.listeners.clear();
  }

  async _runFetch(query, page) {
    // Stamp this fetch; if a newer fetch starts before this one resolves,
    // our generation will be stale and we must discard the result.
