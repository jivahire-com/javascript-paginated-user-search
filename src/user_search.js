...
export class UserSearch {
  constructor(opts) {
    ...
    this._fetchGeneration = 0; // incremented on every fetch; stale responses are dropped
  }
  ...
  setQuery(q) {
    ...
    // Fix (a): read this.page at fire time, not at schedule time.
  ...
  getState() {
    return {
      ...
      users: this.users,
      total: this.total,
      // Fix (c): ceil so a trailing partial page is counted.
  ...
  async _runFetch(query, page) {
    // Fix (b): stamp this fetch; if a newer one starts before this resolves,
